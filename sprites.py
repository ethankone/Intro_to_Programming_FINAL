...
class Player(Sprite):
    ...
    def load_images(self):
        self.standing_frames = [self.game.spritesheet.get_image(382, 645, 174, 126),
                                self.game.spritesheet.get_image(0, 1879, 206, 107)
                                ]
        for frame in self.standing_frames:
            frame.set_colorkey(BLACK)
        self.walk_frames_r = [self.game.spritesheet.get_image(382, 645, 174, 126),
                                self.game.spritesheet.get_image(0, 1879, 206, 107)
                                ]
    ...
    def animate(self):
        ...
        if not self.jumping and not self.walking:
            # this gets the current delta time and checks against 200 milliseconds
            if now - self.last_update > 200:
                self.last_update = now
...
class Cloud(Sprite):
    ...
    def update(self):
        if self.rect.top > HEIGHT * 2: 
            self.kill
            ''' animated clouds and made it so they 
